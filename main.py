    ...
    if files:
        if mode == "打者分析":
            ...
            if not target_df.empty:
                ...
                # --- B. 扇形角度別分布（選択式） ---
                st.markdown("---")
                st.subheader(f"📐 打球角度別 {angle_metric} 分布")
                ...
                if angle_col in target_df.columns:
                    ...
                    for b_idx in range(len(bins)-1):
                        ...
                        if at_bats > 0:
                            if angle_metric == "打率":
                                val = bin_data[result_col].isin(hit_keywords).sum() / at_bats if result_col in bin_data.columns else 0
                            elif angle_metric == "平均飛距離":
                                val = bin_data['Distance'].mean() if 'Distance' in bin_data.columns else 0
                            else: # 平均打球速度
                                val = bin_data['ExitSpeed'].mean() if 'ExitSpeed' in bin_data.columns else 0
                        else:
                            val = 0
                        
                        val_list.append(val if not np.isnan(val) else 0)
                        n_list.append(at_bats)

                    fig_polar = plt.figure(figsize=(10, 6))
                    ax_polar = fig_polar.add_subplot(111, polar=True)
                    bars = ax_polar.bar(theta, val_list, width=np.deg2rad(9.5), color='darkred', alpha=0.7, edgecolor='black', zorder=3)
                    ...
                    for t, v, n in zip(theta, val_list, n_list):
                        if n > 0:
                            if angle_metric == "打率":
                                label = f".{int(v*1000):03d}\n(n={n})"
                            else:
                                label = f"{v:.1f}\n(n={n})"
                            ax_polar.text(t, v + (max(val_list)*0.05), label, ha='center', va='bottom', fontsize=9, fontweight='bold', zorder=10)

                    ax_polar.set_ylim(0, max(val_list) * 1.2 if max(val_list) > 0 else 1.0)
                    # バレルゾーン
                    ax_polar.fill_between(np.deg2rad([25, 35]), 0, max(val_list)*1.2, color='orange', alpha=0.1, zorder=2)
                    st.pyplot(fig_polar)
                    st.caption(f"※半径の長さは{angle_metric}を示しています。")
                else:
                    st.warning("角度データ(Angle)が見つかりません。")
